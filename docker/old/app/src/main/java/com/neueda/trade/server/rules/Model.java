...
public class Model {
    ...
    /**
     * Helper method to convert an Enum type into a string value using its
     * JsonPropetry annotation.
     * 
     * @param ch string to convert from
     * @return
     */
    @SuppressWarnings("unchecked")
	public static <T extends Enum<T>> T fromChar(String ch, Class<T> type) {
    	for (Field field : type.getDeclaredFields()) {
    		JsonProperty json = field.getDeclaredAnnotation(JsonProperty.class);
    		if (ch!= null && json!=null && ch.equals(json.value())) {
    			try {
    	...
    	throw new TradeException("Cannot convert %s to a Buy or Sell value", ch);
    }

    /**
     * Help method to convert Enums to strings
     * 
     * @return string stored in the JsonProperty Annotation
     */
    public static <T extends Enum<T>> String toChar(T enumObj) {
    	for (Field field : enumObj.getClass().getDeclaredFields()) {
    		JsonProperty json = field.getDeclaredAnnotation(JsonProperty.class);
    		try {
    			if (enumObj == field.get(null)) {
    				return json.value();
				}
    		}
			catch (IllegalAccessException ex) {
    	...
    	throw new FatalTradeException("Cannot convert %s to a string value", enumObj);
    }
