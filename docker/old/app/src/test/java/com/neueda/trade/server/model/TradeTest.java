...
public class TradeTest {
	...
	public void testCreateBuyTradeWithValidValues() {
		Date now = new Date();
		Trade trade = factory.createTrade("1234567890", factory.createStock("FTSE.AA"), now, 100.0, 50, BuySell.Buy);
		assertThat(trade.getTransid(), equalTo("1234567890"));		
		assertThat(trade.getStock().getTicker(), equalTo("FTSE.AA"));		
	...
	public void testCreateSellTradeWithValidValues() {
		Date now = new Date();
		Trade trade = factory.createTrade("12345678901", factory.createStock("FTSE.BARC"), now, 101.0, 51, BuySell.Sell);
		assertThat(trade.getTransid(), equalTo("12345678901"));		
		assertThat(trade.getStock().getTicker(), equalTo("FTSE.BARC"));		
		assertThat(trade.getPtime(), equalTo(now));		
	...
	public void testCreateAndValidateBuyTradeWithValidValues() {
		Trade trade = factory.createTrade("1", factory.createStock("FTSE.AA"), new Date(), 100.0, 50, BuySell.Buy);
	...
	public void checkPlaceTradeRejectsInvalidVolume() {
		model.validate(factory.createTrade("1", factory.createStock("FTSE.AA"), new Date(), 100.0, -99, BuySell.Buy));
	}
	...
	public void checkPlaceTradeRejectsInvalidPrice() {
		...
		model.validate(factory.createTrade("1", factory.createStock("FTSE.AA"), new Date(), -1.1, 100, BuySell.Buy));
	}
