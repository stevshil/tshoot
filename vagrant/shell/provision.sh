...
if ! docker pull localhost:5000/mysql
then
...
if ! docker pull localhost:5000/activemq
then
...
docker tag citi/test1 localhost:5000/citi/test1
if ! docker push localhost:5000/citi/test1
then
  echo "Failed to push test image to our local repo"
  exit 8
fi
docker rmi citi/test1

# Remove the tagged images
for image in localhost:5000/citi/test1 localhost:5000/mysql localhost:5000/activemq
do
  docker rmi $image
done
